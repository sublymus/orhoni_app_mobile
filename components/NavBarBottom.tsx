...
export function NavBarBottom() {
    ...
    return <Row style={[styles.container, { backgroundColor: colors.back_item }]}>
        ...
        <TouchableOpacity style={styles.icon_ctn} onPress={()=>route.name=='package'?'':route.name=='index'?router.push('/'):router.replace('/')} >
            <Image source={route.name == 'package' ?
                require(`@/assets/icons/wallet.png`) :
                require(`@/assets/icons/wallet-outline.png`)
            } style={styles.icon} tintColor={colors.color} />
            <ThemedText>Package</ThemedText>
        </TouchableOpacity>
        <TouchableOpacity style={styles.icon_ctn} onPress={()=>route.name=='alert'?'':route.name=='index'?router.push('/'):router.replace('/')} >
            <Image source={route.name == 'alert' ?
                require(`@/assets/icons/megaphone.png`) :
                require(`@/assets/icons/megaphone-outline.png`)
            } style={styles.icon} tintColor={colors.color} />
            <ThemedText>Alert</ThemedText>
        </TouchableOpacity>
        <TouchableOpacity style={styles.icon_ctn} onPress={()=>route.name=='setting'?'':route.name=='index'?router.push('/'):router.replace('/')} >
            <Image source={route.name == 'setting' ?
